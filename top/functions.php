<?php
	...
	$hash = strtolower(md5($names.date("Y")));
	$hash = array_chunk(str_split(md5($names.date("Y")),4),3)[1];
	$hash = implode("-",$hash);
	return $hash;
}
...
function menus(){
	...
	foreach($menus as $k=>$m){/* group not dead */if(!isset($unlisted[$k])){/* member of group */if(in_array($m,$groups)){/* fetch submenus */foreach($subgroups as $j=>$s){if($flip[$m] == $s) { /* subgroup not dead */if(!isset($unlisted[$j])){$nav[$groups[$flip[$m]]][$j] = $menus[$j]; }}}}else{/* fetch unclassified menus */if(!isset($subgroups[$k])){$nav[] = $m;}}}}return $nav;
}
