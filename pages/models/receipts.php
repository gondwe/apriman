...
if(is_string($dv)){
	$topclass = topclass($sid); 
	$thiclass = fetch("select class from students where adm_no = '$dv' and sid = '$sid'"); 
	// var_dump($thiclass);
	$class = (int)$thiclass > (int)$topclass ? $topclass : $thiclass;
...
$tx = getlist("select id, vdat from vdata where vprop = 'terms'");include("studentclass.php");
...
foreach($tx as $t=>$x){$balances[$t] = $p->balance($t);}
$balance_stream = $p->bal_stream();$vb = $p->ydebt;
$bals = $balance_stream["b"]; 
$arrs = $balance_stream["r"];$rec = "";

// spill($arrs);

foreach($rdata as $k=>$rd){
$html = "";$rr = $rd["id"]; $rid = "rec".$rr;if($rd["amount"]=='0'){echo '<style>#'.$rid.'{background-image: url("assets/images/cancelled.png");background-repeat: no-repeat;background-position: center;}</style>';}$rcno = $rd["rcno"] == null ? null : $rd["rcno"];if(is_numeric($rd['arrid'])){$j = new fee($dv);$arr = $j->arrears();}
...
$rec .=  "<tr><td id='rcp2' class=''><b>Reg No : </b>".$dv."<b>  Class : </b>".rx(fetch("select class from finance_payments where id = ".$rd['id'])).$stream."</td><td class=''><b>Term : </b>".rx($tx[$rd['term']])."</td></tr>";
...
$rec .=  "<td id='farr' class='' >"; if(is_numeric($rd['arrid'])){ 
$rec .= "<b>Arrears : </b>".addcommas($arrs[$rd['term']][$rd['id']]);} 
$rec .= "</td></tr>";
...
function rec_voteheads($rd, $vh){$rec = '';foreach($vh as $v){
	$amt = '--'; $cts = '-';
if(is_null($rd["arrid"]) || $rd["arrid"] == 0){
...
$specify = is_null($arrid) || $rd["arrid"] == 0 ? " SPECIFY " : "<strong> $ARCAN </strong>";
$newsh = is_null($arrid)|| $rd["arrid"] == 0? "--" : addcommas($rd['amount'],0);
$newcts = is_null($arrid)|| $rd["arrid"] == 0? "--" : "00";
