<?php 
	class student {
		...
		private function init(){
			...
			$topc = topclass($sid);
			if(!empty($this->sdata)){
				$this->classs = $this->sdata["class"] > $topc ? $topc : $this->sdata["class"] ;
	...
	class fee extends student{
		...
		function bal_stream(){
			...
			$sql = "select ifnull(sum(amount),0) as aramount from finance_arrears ".$this->_asw." and year(date) = year(current_timestamp) and finance_arrears.id in (select arrid from finance_payments as arr where adm_no = '$this->adm')";
			$arrears = fetch($sql); 

			foreach($this->terms as $t=>$tt){
				$bdata[$t] = [];
			...
			foreach($bdata as $t=>$data){
				...
				foreach($data as $k=>$d):
					$preps[$t][$k] =  null;
					$rpreps[$t][$k] =  null;
					if(is_null($_paid_amts[$k]["arrid"]) || $_paid_amts[$k]["arrid"] == 0){
		...
		function arrears(){
			...
			left join finance_payments as fp on fp.arrid = fa.id  and fp.arrid <> 0 and fp.arrid is not null and fp.`class` = '$this->classs' and fp.scode = '$this->scode'
			...
			if(!empty($d)){
					foreach($this->terms as $t=>$n){ 
						foreach($d as $k=>&$v){
							if($v["term"] == $t){
								$vname = $v['names'];
								$tvme[$t][$v["names"]] = fetch("select sum($vname) from finance_payments ".$this->_asw." and term = '$t' and `class` = '".$this->classs."' and (arrid is null or arrid =0) and year(date) = year(current_timestamp)"); 
								unset($d[$k]);
							}
