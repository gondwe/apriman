...
// echo gethash("ober boys boarding");
$tbl = getlist("show tables");
// spill($tbl);


foreach($tbl as $table){
	process("delete from `$table` where id = 0;");
	$sch = getlist("describe $table");
	$idfield = @array_search("id",array_column($sch, "Field",1));
	$key = isset($sch[$idfield]["Key"]) ? $sch[$idfield]["Key"] : "1" ;
	if($key == ""){
		process("ALTER TABLE `$table` modify id int(9) PRIMARY KEY AUTO_INCREMENT NOT NULL");
	}
}

process("delete from finance_arrears where id not in (select arrid from finance_payments where arrid <> 0 or arrid is null);");
