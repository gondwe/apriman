...
$arr_years = getlist("select distinct(year(date)) from finance_payments where year(date) < year(current_timestamp) and adm_no = '$adm'");
// spill($arr_years);
...
foreach($arr_years as $yr){
	...
	$bypaid = fetch("select sum(amount) as tot from finance_payments where (arrid is null or arrid = 0)  and scode = '$sid' and year(date) = '$yr' and adm_no = '$adm' ");
